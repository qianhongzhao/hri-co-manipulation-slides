...
\newcommand{\pinkcomment}[1]{\textcolor{pink}{#1}}
...
\begin{frame}[plain]
  ...
  \hspace{0.78cm}Physical Human--Robot Co-Manipulation\par} \pinkcomment{here, the vspace between two lines are too wide}
...
\begin{frame}{Human intention is hidden but control-relevant \textcolor{pink}{}}
...
\end{alertblock}
\pinkcomment{for the page number, do not contain the backup part insides of total}
\end{frame}
...
\begin{frame}[shrink=5]{Shared papers show intention modeling is feasible \pinkcomment{and benefical}}
\small
\begin{columns}[T,onlytextwidth]
...
\begin{itemize}
  ...
  \item Provides a relevant physical collaborative-transport setting \pinkcomment{this sentence contains only one word in one line avoid this situation}
\end{itemize}
\end{block}
\column{0.315\textwidth}
...
\pinkcomment{Consider added this in one of the above option or form the followed as a new one "把 human model 做成“interaction primitive model”

我认为这对你们的 generalization 目标非常重要。

多数 co-manipulation 看起来任务不同，但底层反复出现的 interaction primitives 很有限，例如：

m
t
	​

∈{translate,rotate,hold,pivot,align,lift,lower,insert,yield}

然后每个 primitive 再有连续参数：
...
：human 的 role/compliance。

例如“搬桌子过门”可以分解为：

translate→rotate→align→translate

“协同插入”则是：

approach→align→hold→insert

模型不为每个任务训练一套新 policy，只学习：

通用 primitive 库；
primitive 的转换规律；
当前 primitive 的连续目标参数。

这可以写成 switching latent-state model：
...
它类似 controlled HMM、switching dynamical system 或 neural state-space model。ADACORL 也是类似思想：不一定用 cost 表示人，而是联合学习 human policy 和 latent-state dynamics，并将 human model 与具体 task model 分离。"}
\end{frame}
...
\pinkcomment{所以最好把它作为低层输出，而不是完整模型。}

\pinkcomment{Find some references, consider clarify what is the label? for this kind of models, the indivdual label may not good enough as you have multiple vaild trajs, right? }
\end{frame}
...
\pinkcomment{Again, how to train this model? Can we have label? looks this is something like WM that we do not have the latent state label of the systems for control right? how to address this? }
\end{frame}
...
\end{columns}
\pinkcomment{what is the difference between C and B? also the key point is how to train this model? }
\end{frame}
...
\end{itemize}
\pinkcomment{This one looks the same as this paper right "Diffusion Co-Policy for Synergistic Human-Robot Collaborative Tasks", advantage / disadvantage? }
\end{frame}
...
\end{block}
\pinkcomment{so many components are overlaped in the flowchart plot }
\end{frame}
...
\end{alertblock}
\pinkcomment{ One thing also need to consider: how to get data? exisiting dataste? collect on their own and form it as a benchmark or dataset paper? how many data modality?}
\end{frame}
