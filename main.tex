...
\begin{frame}[plain]
  ...
  \hspace{0.78cm}Physical Human--Robot Co-Manipulation\par}
    ...
    \pinkcomment{The left blue vertical line looks too long}
...
\begin{frame}{Frame Title}
    \pinkcomment{Use one page to revisit the cominipulation task definied the "Diffusion copolicy " paper, which is the test setting he mentioned in the email. what data is available, why human intention is necessary to model}
\end{frame}
...
``Multimodal behavior'' means multiple plausible future human strategies. 
\end{block}
\end{frame}
...
\begin{itemize}
  \item Generates multiple plausible (\pinkcomment{this kind of words (like plausible) I cannot understood, try to avoid it}) joint force/action sequences
...
\begin{frame}{Five control-relevant intention representations}
\footnotesize
\begin{columns}[T,onlytextwidth]
\column{0.48\textwidth}
\begin{block}{Interaction primitive}
...
\end{block}
\end{columns}
\pinkcomment{If the intentions are considered as hiriachy, form this page as hiraichy(multilayers) for example primitive -> 4 options. Do not form them as the paralelly in the same layer}
\vspace{0.12cm}
\begin{alertblock}{Selection criteria}
Identifiability, predictive accuracy, real-time control utility, and transfer across tasks. \pinkcomment{too vague and short, consider use one page to explain the metrics in more details}
\end{alertblock}


\end{frame}

\begin{frame}[shrink=7]{High-level option: Learn reusable interaction primitives}
\begin{columns}[T,onlytextwidth]
...
\end{exampleblock}
\begin{alertblock}{How it can be trained}
Use weak task-phase labels when available and semi-supervised EM / state-space learning otherwise. Train continuous parameters from future motion and wrench prediction. \pinkcomment{1. "EM" $\rightarrow$ this kind of abbv. need to be definied before you use them. 
2. for differet primitive, how to identifiy? how many primitive should prepare.
}
\end{alertblock}
\end{columns}
\vspace{0.15cm}
{\scriptsize Related idea: ADACORL learns factored human decision models with limited labels (Unhelkar et al., CoRL 2020).}
\end{frame}

\begin{frame}[shrink=6]{Option A: Predict the human's desired object motion}
\begin{columns}[T,onlytextwidth]
\column{0.52\textwidth}
...
\end{columns}
\vspace{0.05cm}
{\scriptsize Best viewed as a \textbf{low-level control output}, not a complete human model (Townsend et al., 2017; Franceschi et al., 2023).}
\pinkcomment{The scale of this page is different from other pages, you can split this page into two pages to ensure the scale of the pages are identical}
\end{frame}
...
\begin{frame}[shrink=7]{Option C: Model intention as a local vector field}
\begin{columns}[T,onlytextwidth]
\column{0.52\textwidth}
...
\end{block}
\begin{alertblock}{Training without parameter labels}
Fit object-motion likelihood by constrained system identification. Infer $\theta_H$ and switching modes with EM or a particle filter; impose stability and feasibility on $f_H$.
\end{alertblock}
\end{columns}
\vspace{0.08cm}
{\scriptsize Related: constraint-aware DS intent estimation with particle filtering (Shao et al., 2024).}
\pinkcomment{{The scale of this page is different from other pages, you can split this page into two pages to ensure the scale of the pages are identical}}
\end{frame}
